...
describe("BrowserLeasePool", () => {
  ...
  it("never reclaims an arbitrary owner through the shared browser handle", async () => {
    ...
    expect(await pool.releaseUnleased(value)).toBe(false);
    expect(pool.forceReleaseUnleased(value)).toBe(false);
