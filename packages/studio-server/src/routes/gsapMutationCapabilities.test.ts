...
describe("GSAP writer capability matrix", () => {
  ...
  it("keeps the Acorn dispatcher free of Recast and gates hold sync with the selected writer", () => {
    ...
    const routeBody = source.slice(source.indexOf("export function registerFileRoutes"));
    expect(acornBody).not.toContain("loadGsapParser");
    expect(routeBody).toContain('writer === "acorn"');
    expect(routeBody).toContain("? syncPositionHoldsBeforeKeyframes(newScript)");
    expect(routeBody).toContain("(await loadGsapParser()).syncPositionHoldsBeforeKeyframes");
  });
