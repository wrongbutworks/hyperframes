...
export function resolveZoneDropPlacement(input: {
  ...
  if (placement.needsInsert) {
    ...
    const below = desiredRow >= 0 ? desiredRow + 1 : order.length;
    ...
    const above = desiredRow >= 0 ? desiredRow : 0;
    const insertRow =
      preferInsertAbove && desiredRow >= 0 && isInsertAllowedForZone(above, audioRow, isAudio)
        ? above
        : below;
