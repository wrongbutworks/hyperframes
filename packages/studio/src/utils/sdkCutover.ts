...
async function dispatchGsapOpAndPersist(
  ...
  // persistSdkCandidateMutation owns SDK serialization and, when provided, also
  // enters the serializer shared with the legacy GSAP path.
  const run = async (): Promise<CutoverResult> => {
    try {
      const serializedBefore = session.serialize();
      const undoBefore = await captureOnDiskBefore(deps, targetPath, serializedBefore);
      // This function takes the shared GSAP/legacy lock around the complete
      // read-modify-write below. Remove the serializer from the nested deps so
      // persistSdkCandidateMutation does not recursively acquire the same key.
      const transactionDeps = deps.serialize ? { ...deps, serialize: undefined } : deps;
      const result = await persistSdkCandidateMutation(
        session,
        targetPath,
        undoBefore,
